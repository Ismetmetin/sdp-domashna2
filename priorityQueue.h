...
public:
    ...
    {
        if (this->priorityToNodeIndex.find(priority) != priorityToNodeIndex.end())
        {
    ...
    void serialize(std::ofstream &str) const // опашка-> поток
    ...
    {   
        if(str.bad()) {std::cerr << "Bad stream"; return;}
    ...
    void deserialize(std::ifstream &out) // поток -> опашка
    ...
    // strong exception safety
    {
        ...
        std::swap(this->binaryHeap, copyHeap); //mai
        std::swap(this->priorityToNodeIndex,copyToNodeIndex);
        std::swap(this->elementCount,copyElementCount);
        std::swap(this->heapSize, copyHeapSize);
    }
